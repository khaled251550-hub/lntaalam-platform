...
const arabicLessons = [...arabicLetterLessons, ...arabicFoundationLessons];
const englishLessons = [...englishLetterLessons, ...englishGrammarLessons];
...
function LanguageLessonPage({ language }: { language: 'english' | 'arabic' }) {
  ...
  const speakLesson = () => {
    ...
    const speech = new SpeechSynthesisUtterance(isArabic ? `${lesson.symbol}. ${lesson.explanation}` : `${lesson.symbol}. ${englishWord}`);
  ...
  return <CatalogShell>
    ...
    <PageIntro eyebrow={lesson.kind === 'letter' ? (isArabic ? 'حرف عربي' : 'حرف إنجليزي') : (isArabic ? 'قاعدة عربية' : 'قاعدة إنجليزية')} title={lesson.title} description={lesson.description} action={<div className="flex flex-wrap gap-2"><Button variant="outline" onClick={speakLesson} data-testid={`button-speak-${language}`}><Volume2 size={17} />استمع للنطق</Button><span className="grid h-12 min-w-12 place-items-center rounded-xl bg-accent px-3 text-xl font-black text-accent-foreground">{lesson.symbol}</span></div>} />
